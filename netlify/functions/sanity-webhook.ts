...
export const handler = async (event: any) => {
  ...
  const secretHeader = event.headers['sanity-webhook-secret'] || event.headers['x-sanity-webhook-secret'];
  ...
  try {
    ...
    if (!_id || !_type) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'Invalid payload: missing _id or _type' }),
      };
    }
    ...
    if (operation === 'delete' || payload.transition === 'disappear') {
      ...
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ success: true, action: 'deleted', table: targetTable, id: _id }),
      };
    }
    ...
    switch (_type) {
      ...
      default:
        return {
          statusCode: 200,
          headers,
          body: JSON.stringify({ message: `Ignored unhandled document type: ${_type}` }),
        };
    }
    ...
    if (syncResult?.error) {
      ...
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ error: syncResult.error.message }),
      };
    }
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ success: true, action: 'upserted', type: _type, id: _id }),
    };
  } catch (err: any) {
